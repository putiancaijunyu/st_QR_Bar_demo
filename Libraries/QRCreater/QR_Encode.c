...
int EncodeSourceData(char *lpsSource, int ncLength, int nVerGroup)
{
	...
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
	{
		...
		{//如果数据块是数字模式
			...
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned int)m_nBlockLength[i], nIndicatorLenNumeral[nVerGroup]);

			for (j = 0; j < m_nBlockLength[i]; j += 3)
			{
				if (j < m_nBlockLength[i] - 2)
				{
					wBinCode = (unsigned int)(((lpsSource[ncComplete + j]	  - '0') * 100) +
				...
				{
					wBinCode = (unsigned int)(((lpsSource[ncComplete + j] - '0') * 10) +
				...
				{
					wBinCode = (unsigned int)(lpsSource[ncComplete + j] - '0');
		...
		else if (m_byBlockMode[i] == QR_MODE_ALPHABET)
		{
			...
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned int)m_nBlockLength[i], nIndicatorLenAlphabet[nVerGroup]);

			for (j = 0; j < m_nBlockLength[i]; j += 2)
			{
				if (j < m_nBlockLength[i] - 1)
				{
					wBinCode = (unsigned int)((AlphabetToBinaly(lpsSource[ncComplete + j]) * 45) +
				...
				{
					wBinCode = (unsigned int)AlphabetToBinaly(lpsSource[ncComplete + j]);
		...
		else if (m_byBlockMode[i] == QR_MODE_8BIT)
		{
			...
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned int)m_nBlockLength[i], nIndicatorLen8Bit[nVerGroup]);

			for (j = 0; j < m_nBlockLength[i]; ++j)
			{
				m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned int)lpsSource[ncComplete + j], 8);
			}
		...
		{//日本模式
			...
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned int)(m_nBlockLength[i] / 2), nIndicatorLenKanji[nVerGroup]);

			for (j = 0; j < m_nBlockLength[i] / 2; ++j)
			{
				unsigned int wBinCode = KanjiToBinaly((unsigned int)(((BYTE)lpsSource[ncComplete + (j * 2)] << 8) + (BYTE)lpsSource[ncComplete + (j * 2) + 1]));
...
//			for (j = 0; j < m_nBlockLength[i] / 2; ++j)
//			{
//				unsigned int wBinCode = ChineseToBinaly((unsigned int)(((BYTE)lpsSource[ncComplete + (j * 2)] << 8) + (BYTE)lpsSource[ncComplete + (j * 2) + 1]));
...
//设置bit流
int SetBitStream(int nIndex, unsigned int wData, int ncData)
{
	int i;
...
unsigned int KanjiToBinaly(unsigned int wc)
{
	...
	return (unsigned int)(((wc >> 8) * 0xc0) + (wc & 0x00ff));
}
/*
二、中文汉字的与日文汉字转换步骤相似：
...
unsigned int ChineseToBinaly(unsigned int wc)
{
	...
	{
		return (unsigned int)((((wc >> 8)-0xa1) * 0x60) + ((wc & 0x00ff)-0xa1));
	}
	if(wc >= 0xb0a1 && wc <= 0xf9fe)
	{
		return (unsigned int)((((wc >> 8)-0xa6) * 0x60) + ((wc & 0x00ff)-0xa1));
	}
	return (unsigned int)((((wc >> 8)-0xa6) * 0x60) + ((wc & 0x00ff)-0xa1));
}
//纠错码
